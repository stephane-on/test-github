...
for i=1:length(info_peaks_t{1,1})
    ...
    [f2,spec2,df,norm]=fourier_transform(data_filter(round(tsbeg/S.DELTA):round(tsend/S.DELTA)),S.DELTA,NFFT,1,length(data(round(tsbeg/S.DELTA):round(tsend/S.DELTA))));
    % Take out f=0 Hz
    f=f2(2:length(f2));
    vel_spec_S=abs(spec2(2:NFFT/2+1));
    [fn2,specn2,df,norm]=fourier_transform(data_filter(round(tnbeg/S.DELTA):round(tnend/S.DELTA)),S.DELTA,NFFT,1,length(data(round(tnbeg/S.DELTA):round(tnend/S.DELTA))));
