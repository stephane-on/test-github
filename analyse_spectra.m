...
answer=inputdlg({'Enter gamma:','Enter Qo:','Enter alpha:','Enter vs (m/s):'},'Give attenuation parameters',1,{'1.0','3000.0','0.0','3500.0'});
...
for i=1:length(info_peaks_f{1,1})
    ...
    if fmax <= fmin
        ...
        if isempty(index) ~= 1
            ...
            fprintf(fid_out2,'%s\n','>');
            fprintf(fid_out2,'%f %e\n',[A(index,1) smooth(acc_spec_source,length(acc_spec_source)/10)]');
        else
